    ...
    *Example* ::
    ...
    for col in range(data.shape[1]):
        if col in [GazePointXLeft, GazePointYLeft, PupilLeft,
                   GazePointXRight, GazePointYRight, PupilRight,
                   GazePointX, GazePointY]:
            new_data[:,col] = nan_moving_median_1d(data[:,col], n)
        else:
            new_data[:,col] = np.copy(data[:,col])
...
def nan_moving_median_1d(data, n=3):
    v = np.ones(n)/n

    if n%2==0:
        offset = int(n/2)
        f = [np.nanmedian(data[max(idx-offset,0):idx+offset]) for idx in range(len(data))]
    else:
        offset = int((n-1)/2)
        f = [np.nanmedian(data[max(idx-offset,0):idx+offset+1]) for idx in range(len(data))]
...
def detect_fixation_vt_tobii(data, max_velocity=100, min_duration=100, eye='LR'):
    """
    Detect fixations using velocity-threshold method.
    Returned value is a numpy.ndarray with following 4 columns.
